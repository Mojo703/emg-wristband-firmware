...
| [0022](0022-what-the-decision-layer-cannot-buy.md) | What the decision layer cannot buy | S1.1 scored for the first time: rest false positives pass (0 in both regimes), false negatives and misclassification fail everywhere (7.4%/12.9% pooled); six decision-layer ideas move errors sideways because one window is worth ~75% and the spine already matches an oracle detector; the levers that move are vocabulary (sparse 4-atom sequences: wrong fires 6.9% → 0%) and the thumb modifier (misclass 0.0%, FN 8.0%, 3.8% false fires, but only with same-don thumb-down calibration) |
| [0023](0023-guided-calibration-refactor-findings.md) | Guided calibration refactor findings | Historical recipe evidence and rejected host-paced cue transaction. Tuesday now uses a device-anchored whole-song schedule, midpoint timing projection, heartbeat/interruption lifecycle, and resident activation; final BLE, Timing, command/anti, reboot, and interruption checks remain hardware acceptance gates. |
| [0024](0024-calibration-schedule-contract-regression.md) | Calibration schedule contract regression | Generator v3 violated firmware's 500 ms recovery contract; shared timing constants and a producer-to-real-consumer integration test prevent recurrence. |
| [0025](0025-product-gesture-vocabulary-and-center-negatives.md) | Product gesture vocabulary and center negatives | Center finger extension poisoned ulnar recall (71/90 ulnar rows became negative); the praised 10-class grip model is restored as the canonical golden contract and resident calibration. |
