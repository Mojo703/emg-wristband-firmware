...
use ads1298::{Ads1298Device, ChipRole};
...
fn single_chip_test() -> Result<()> {
    ...
    let spi_driver = SpiDriver::new(
        peripherals.spi2,
        pins.gpio12, //SCLK //White
        pins.gpio9, //MOSI // Purple
        Some(pins.gpio10), //MISO //Orange
        &SpiDriverConfig::new(),
    )?;
    ...
    let spi_config = SpiConfig::new()
        .baudrate(2000000.Hz())
        .data_mode(esp_idf_svc::hal::spi::config::MODE_1);
    let spi_dev1 = SpiDeviceDriver::new(&spi_driver, Some(pins.gpio8), &spi_config)?; //CS //Grey

    let mut clk_sel = PinDriver::output(pins.gpio11)?;
    clk_sel.set_high()?;
    
    

    let reset_n_a = PinDriver::output(pins.gpio6)?; //yellow
    let pwdn_a = PinDriver::output(pins.gpio7)?; //Brown
    let start = PinDriver::output(pins.gpio2)?; //Green
    let drdy1 = PinDriver::input(pins.gpio4, Pull::Floating)?; //BLue
