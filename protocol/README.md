...
| `CalibrationTimingLoopStatus` | device → host | Tagged `Stopped` or `Running` state. Only `Running` carries the current colour, phase progress, device monotonic cycle anchor, and observation instant, so an idle report cannot retain stale loop evidence. The backend uses this with its clock probes; the device never follows a host clock. |
| `CalibrationTimingStatus` | backend → browser | Browser projection with a tagged `Stopped`/`Starting`/`Running`/`Stopping`/`Error` phase and a tagged `NoSamples`/`Measured` estimate. Running and stopping carry their required device observations, error carries its detail, and all estimate statistics arrive together. Signed manual trim remains orthogonal; total correction is derived as automatic offset plus trim. The rolling capacity is 11 and state is volatile per device. |
...
| `CalibrationScheduleUploadAcknowledged` | device → host | Exact upload identity plus a tagged `Begin` or `Chunk { first_entry }` operation. Each variant owns the semantic fingerprint for that operation, so a Begin cannot be represented with a chunk-index sentinel or advance a different payload. |
| `CalibrationScheduleCommitDeferred` | device → host | Exact run/revision plus the typed retry reason `PreparationIncomplete` or `ScheduleNotAnchored`. Only this frame authorizes a Commit retry; free-form bench error text does not drive state transitions. |
...
| `CalibrationCandidateStatus` / `CalibrationResidentActivated` | device → host | A tagged `Absent` or `Present { content_identity, total_count, validity }` candidate state, followed by acknowledgement that a valid candidate became the active resident calibration. An absent candidate cannot carry independent identity or numerical/CRC validity. |
