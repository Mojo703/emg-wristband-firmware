...
pub enum Frame {
    ...
    /// Host -> device: stream the complete active resident slot. The transfer is
    /// read-only and is refused while an acquisition song is running.
    CalibrationExportRequest { transfer_id: u32 },

    /// Device -> host: one contiguous piece of the active resident slot. Joining
    /// chunks by `offset` produces the exact 192 KiB flash image, including its
    /// fitted model, collected rows, and integrity words.
    CalibrationExportChunk {
        transfer_id: u32,
        sequence: u32,
        offset: u32,
        total_bytes: u32,
    ...
    /// Device -> host: the requested resident cannot be exported.
    CalibrationExportFailed { transfer_id: u32, reason: String },
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum GuidedCalibrationModifier {
    Command,
    CenterExtension,
    GripSoft,
    GripMedium,
...
pub struct GuidedCalibrationCue {
    ...
    pub modifier: GuidedCalibrationModifier,
}
...
fn calibration_schedule_operation_fingerprint(
    ...
    for entry in entries {
        ...
        crc.update(&[match entry.modifier {
            CalibrationModifier::Command => 0,
            CalibrationModifier::CenterExtension => 1,
            CalibrationModifier::GripSoft => 2,
            CalibrationModifier::GripMedium => 3,
            CalibrationModifier::GripHard => 4,
...
/// A complete generated song currently has 52 cues, so chunks must be large enough to
...
/// Performance condition in the interleaved schedule. Commands use the named
/// wrist direction. Golden grip-negative prompts render in one center lane,
/// while their gesture field selects the paired internal prototype.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum CalibrationModifier {
    Command,
    /// Retained only to decode the rejected generation-8 experiment. Current
    /// schedules never emit it because it collided with ulnar deviation.
    CenterExtension,
    GripSoft,
    GripMedium,
    GripHard,
    /// Legacy schedule spelling retained only so persisted v2/v4/v5/v6 tracks
    /// can be decoded and regenerated by a current dashboard.
    ThumbUp,
    /// Legacy schedule spelling; current generation maps this condition to
    /// [`Self::GripSoft`].
    ThumbDown,
}
