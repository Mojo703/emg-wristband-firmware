...
fn parameter_count(var_map: &VarMap) -> usize {
    var_map.all_vars().iter().map(|var| var.as_tensor().elem_count()).sum()
}
...
fn accuracy_on_indices(
    ...
    for chunk in indices.chunks(batch_size) {
        ...
        let predictions = model.forward(&inputs, false)?.argmax(D::Minus1)?.to_vec1::<u32>()?;
        let truth = labels.to_vec1::<u32>()?;
        correct += predictions.iter().zip(&truth).filter(|(predicted, actual)| predicted == actual).count();
    }
...
fn run_pretrain(
    ...
    println!("encoder T'={encoder_time_steps}; interpolating pose {} → {encoder_time_steps}", dataset.frames);

    let mut optimizer = AdamW::new(var_map.all_vars(), ParamsAdamW { lr: learning_rate, ..Default::default() })?;
    ...
    for epoch in 0..epochs {
        ...
        for chunk in order.chunks(batch_size) {
            ...
            let normalized = pose_sequence.broadcast_sub(&mean)?.broadcast_div(&std_dev)?;
            let target = normalized.transpose(1, 2)?.contiguous()?.broadcast_matmul(&interpolation)?; // [B,D,T']
        ...
        println!("epoch {epoch:>3}  pose_mse(z) {:.4}", running_loss / num_batches as f32);
    }
...
fn run_train(
    ...
    let var_map = VarMap::new();
    let var_builder = VarBuilder::from_varmap(&var_map, DType::F32, &device);
    let model = TdsNet::new(Config::classify(train_set.channels, num_classes), var_builder)?;
    println!("params: {:.2}M", parameter_count(&var_map) as f64 / 1e6);
    if let Some(checkpoint) = &init {
        let loaded = load_matching_tensors(&var_map, checkpoint, &device)?;
        println!("initialized {loaded} encoder tensors from {}", checkpoint.display());
    }
    if let Some(parent) = out.parent() {
        std::fs::create_dir_all(parent)?;
    }
    ...
    let warp_basis = if augment_config.needs_basis() {
        Some(augment::warp_basis(augment_config.num_knots, train_set.time, &device)?)
    } else {
        None
    };
    ...
    let mut optimizer = AdamW::new(
        var_map.all_vars(),
        ParamsAdamW { lr: learning_rate, weight_decay: WEIGHT_DECAY, ..Default::default() },
    )?;
    ...
    for epoch in 0..epochs {
        if balance {
            let per_class_count = class_groups.iter().map(|group| group.len()).min().unwrap_or(0);
            order.clear();
        ...
        for chunk in order.chunks(batch_size) {
            ...
            if augment_config.enabled() {
                inputs = augment::apply(&inputs, &augment_config, warp_basis.as_ref(), &mut rng, &device)?;
            }
        ...
        let test_accuracy = accuracy_on_indices(&model, &test_set, &test_indices, batch_size, &device)?;
        let train_accuracy = accuracy_on_indices(&model, &train_set, &fit_indices, batch_size, &device)?;
...
fn main() -> Result<()> {
    match CommandLine::parse().command {
        ...
        Command::Pretrain { data_dir, epochs, batch_size, learning_rate, out } => {
        ...
        Command::Train {
            ...
            validation_subjects,
            seed,
            if augment { augment::AugmentConfig::on() } else { augment::AugmentConfig::off() },
            init,
            n_commands,
            balance,
        ),
