...
pub(crate) struct Dataset {
    pub(crate) x: Tensor, // [N,1,C,T] on device
    pub(crate) y: Tensor, // [N] u32 on device
    pub(crate) n: usize,
    pub(crate) channels: usize,
    pub(crate) time: usize,
    /// Per-window subject id from `{split}_meta.npy` (empty if absent).
    pub(crate) subject: Vec<i64>,
}
...
impl Dataset {
    ...
    pub(crate) fn batch(&self, idx: &[u32], device: &Device) -> Result<(Tensor, Tensor)> {
...
pub(crate) struct PoseSeqDataset {
    pub(crate) x: Tensor,   // [N,1,C,T]
    pub(crate) seq: Tensor, // [N,P,pose_dim]
    pub(crate) n: usize,
    pub(crate) channels: usize,
    pub(crate) time: usize,
    pub(crate) frames: usize,
    pub(crate) pose_dim: usize,
}
...
impl PoseSeqDataset {
    ...
    pub(crate) fn batch(&self, idx: &[u32], device: &Device) -> Result<(Tensor, Tensor)> {
        let sel = Tensor::from_vec(idx.to_vec(), idx.len(), device)?;
        Ok((
            self.x.index_select(&sel, 0)?,
            self.seq.index_select(&sel, 0)?,
        ))
    }
