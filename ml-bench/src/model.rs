...
pub struct TestData {
    pub input: Act,
    pub input_scale: f32,
    pub expected_emb: [f32; EMBED_DIM],
}
...
impl<'a> Cursor<'a> {
    ...
    fn i8_slice(&mut self, n: usize) -> &'a [i8] {
        let s = unsafe {
            core::slice::from_raw_parts(self.data[self.pos..].as_ptr() as *const i8, n)
        };
        self.pos += n;
        s
    }
...
impl Model {
    ...
    pub fn real() -> Self {
        let mut c = Cursor::new(MODEL_BIN);
        ...
        for _ in 0..n_blocks {
            ...
            blocks.push(Block {
                ...
                dw: dw_w,
                dw_bias: dw_b,
                dw_rq: Requant { mult: dw_mult, shift: dw_shift, relu: false },
                pw: pw_w,
                pw_bias: pw_b,
                pw_rq: Requant { mult: pw_mult, shift: pw_shift, relu: false },
                gelu: Some(gelu_lut),
            });
        }
        ...
        Model {
            ...
            proj: proj_w,
            proj_bias: proj_b,
            proj_rq: Requant { mult: proj_mult, shift: proj_shift, relu: false },
            head: None,
            input_len,
            proj_scale,
        }
    ...
    pub fn load_test_data() -> TestData {
        ...
        let input = Act::from_i8_slice(test_input_i8, window, INPUT_CH);
    ...
    pub fn forward(&self, input: &Act) -> ForwardResult {
        ...
        let emb_i8 = layers::linear(
            pooled.as_slice(), &self.proj, &self.proj_bias, EMBED_DIM, self.proj_rq,
        );
        ...
        match &self.head {
            ...
            None => {
                let emb_f32 = l2_normalize_i8(emb_i8.as_slice(), self.proj_scale);
                ForwardResult::Embedding(emb_f32)
            }
        }
    }

    pub fn forward_profiled(&self, input: &Act, p: &mut Profile) -> ForwardResult {
        let mut cur: Option<Act> = None;
        ...
        let emb_i8 = p.time(9, || {
            layers::linear(pooled.as_slice(), &self.proj, &self.proj_bias, EMBED_DIM, self.proj_rq)
        });
        let result = p.time(10, || match &self.head {
            Some((hw, hb)) => {
                ForwardResult::Logits(layers::linear_i32(emb_i8.as_slice(), hw, hb, NUM_CLASSES))
            }
            None => ForwardResult::Embedding(l2_normalize_i8(emb_i8.as_slice(), self.proj_scale)),
        });
        p.iters += 1;
        result
    }
